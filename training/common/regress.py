...
def _get_lr_scheduler(args, kv):
    epoch_size = args.num_examples // args.dataloader_dilation // args.batch_size
...
def _load_optimizer_states(model, args, rank=0):
    if 'load_epoch' not in args or args.load_epoch is None:
        return
    assert args.model_prefix is not None
    model_prefix = args.model_prefix
    if rank > 0 and os.path.exists("%s-%d-symbol.json" % (model_prefix, rank)):
        model_prefix += "-%d" % (rank)
    state_name = '%s-%04d.states' % (model_prefix, args.load_epoch)
    if os.path.exists(state_name):
        logging.info('Loading optimizer states from file %s' % state_name)
        model.load_optimizer_states(state_name)
    else:
        logging.warn('Cannot find optimizer states! File %s does not exist!' % state_name)
...
def _save_model(mod, args, rank=0):
    ...
    if not os.path.exists(dst_dir):
        os.makedirs(dst_dir)
    prefix = args.model_prefix if rank == 0 else "%s-%d" % (args.model_prefix, rank)
    return mx.callback.module_checkpoint(mod, prefix, save_optimizer_states=True)
...
#                 bins=np.linspace(-2.4, 2.4, 49),
    ...
    _load_optimizer_states(model, args, kv.rank)
...
#     initializer = mx.init.Xavier(factor_type="in", magnitude=2.34)
    ...
    if args.lr_finder_start > 0:
        ...
        # save model
        checkpoint = None if args.dryrun else _save_model(model, args, kv.rank)
        model.fit(train,
            begin_epoch=args.load_epoch if args.load_epoch else 0,
